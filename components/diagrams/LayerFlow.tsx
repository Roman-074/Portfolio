...
/** Top-to-bottom architecture diagram: each layer is a row of nodes, joined by connectors. */
    ...
    <ol className="layer-flow" aria-label={title}>
      {layers.map((layer) => (
        <li className={layer.accent ? "layer layer--accent" : "layer"} key={layer.label}>
          ...
          <ul className={layer.linked ? "layer__nodes layer__nodes--linked" : "layer__nodes"}>
